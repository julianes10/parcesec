...
def home():
    ...
    st=getStatus()
    rt=render_template('index.html', title="parcesec Site",status=st,timelapse=timelapse)
    return rt
...
def main(configfile):
  print('parcesec-start -----------------------------')

  # Loading config file,
  # Default values
  cfg_log_traces="parcesec.log"
  cfg_log_exceptions="parcesece.log"
  ...
  helper.internalLogger.critical('parcesec-start -------------------------------')  
  helper.einternalLogger.critical('parcesec-start -------------------------------')
  ...
  try:    
    ...
    helper.einternalLogger.exception(e)  
    print('parcesec-General exeception captured. See log:{0}',format(cfg_log_exceptions))        
    mountBindMediaPath(False)
    loggingEnd()
...
'''----------------       loggingEnd      -------------------'''
def loggingEnd():      
  helper.internalLogger.critical('parcesec-end -----------------------------')        
  print('parcesec-end -----------------------------')
